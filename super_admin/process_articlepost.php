...
if (isset($_SESSION['id'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($action === 'create') {
            ...
            $query = "INSERT INTO article_post (user_id, title, content, image) VALUES (?, ?, ?, ?)";
        ...
        if ($action === 'edit') {
            ...
            if ($image_path) {
                if ($user_role === 'admin') {
                    $query = "UPDATE article_post SET title = ?, content = ?, image = ? WHERE id = ?";
                    $stmt = $conn->prepare($query);
                    $stmt->bind_param("sssi", $title, $content, $image_path, $id);
                } else {
                    $query = "UPDATE article_post SET title = ?, content = ?, image = ? WHERE id = ? AND user_id = ?";
                    $stmt = $conn->prepare($query);
                    $stmt->bind_param("sssii", $title, $content, $image_path, $id, $user_id);
                }
            } else {
                if ($user_role === 'admin') {
                    $query = "UPDATE article_post SET title = ?, content = ? WHERE id = ?";
                    $stmt = $conn->prepare($query);
                    $stmt->bind_param("ssi", $title, $content, $id);
                } else {
                    $query = "UPDATE article_post SET title = ?, content = ? WHERE id = ? AND user_id = ?";
                    $stmt = $conn->prepare($query);
...
if ($action === 'delete') {
    ...
    if ($user_role === 'admin') {
        $query = "DELETE FROM article_post WHERE id = ?";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("i", $id);
    } else {
        $query = "DELETE FROM article_post WHERE id = ? AND user_id = ?";
        $stmt = $conn->prepare($query);
